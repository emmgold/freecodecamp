...
echo "$($PSQL "select distinct(course) from courses full join majors_courses using(course_id) full join students using(major_id) where (first_name = 'Obie' and last_name = 'Hilpert') or student_id is null order by course desc;")"
